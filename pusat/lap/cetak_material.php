...
$pdf->Cell(3.5, 0.8, 'Id Material', 1, 0, 'C');
$pdf->Cell(2.5, 0.8, 'Nama Material', 1, 0, 'C');
$pdf->Cell(2.5, 0.8, 'Jumlah Material', 1, 0, 'C');
$pdf->Cell(3.5, 0.8, 'Satuan', 1, 0, 'C');
$pdf->Cell(2.5, 0.8, 'status', 1, 0, 'C');
$pdf->Cell(3.5, 0.8, 'Tanggal', 1, 0, 'C');
$pdf->Cell(9, 0.8, 'PJ Petugas', 1, 1, 'C');
$pdf->Cell(9, 0.8, 'PJ Lapangan', 1, 1, 'C');
$pdf->SetFont('Arial','',9);
$no=1;

$tanggal_awal=$_POST['dari'];
$tanggal_akhir=$_POST['sampai'];
$sql = mysql_query("SELECT * FROM lap_pusat WHERE tanggal  between  '$tanggal_awal' AND '$tanggal_akhir';");
while($d = mysql_fetch_array($sql)){

    $pdf->SetFillColor(224,235,255);
    $pdf->Cell(1, 0.8, $no, 1, 0, 'C');
    $pdf->Cell(3.5, 0.8, $d['id_material'], 1, 0,'C',true);
    $pdf->Cell(2.5, 0.8, $d['nama_material'],1, 0, 'C');
    $pdf->Cell(2.5, 0.8, $d['jumlah_material'], 1, 0,'C',true);
    $pdf->Cell(3.5, 0.8, $d['jenis_satuan_material'],1, 0, 'C');
    $pdf->Cell(2.5, 0.8, $d['status_barang'],1, 0, 'C',true);
    $pdf->Cell(3.5, 0.8, $d['tanggal'],1, 0, 'C');
    $pdf->Cell(9, 0.8, $d['pj_petugas'], 1, 1,'C',true);
    $pdf->Cell(9, 0.8, $d['pj_lapangan'], 1, 1,'C',true);
